...
- **阶段八**（团队绑定属地隔离）：发音人凭**团队码**绑定省+市+区县属地（一码一区县），**只能看到/录制本区县任务**（服务端按省+市+区县严格过滤任务/词条/上传，客户端无法绕过）；管理后台新增「团队管理」页（团队码 CRUD）+「发音人管理」属地纠错（改属地自动解除团队绑定）+「任务分配」**关联团队码**（投放区划由团队码自动带出，含区县），省管理员仅能管理本省团队码/发音人/团队任务。
...
| 资料编辑页 | `pages/profile` | **头像（`chooseAvatar`，**合规整改后仅本地缓存展示，不上传服务器**）/ 姓名（`type=nickname`，**存服务器供后台识别**）/ 属地（团队码绑定，**只读**展示「省·市」）/ 性别 / 年龄段**，保存走 `POST /api/mp/profile`（**不含 `province_code`**——属地唯一来源是团队码绑定） |
...
### 阶段八·A：团队绑定属地隔离（省+市+区县，已实现）

**目标**：发音人**什么地方的只能录什么地区的语音**——不是本地区的任务既**不能录也看不到**。

**方案（团队码绑定，一码一区县）**：
- 管理后台新增「**团队管理**」页（`/teams` 路由）：每个团队码唯一绑定**一个省+市+区县**（`UNIQUE(province_code, city_code, district_code)`，三级必选），发音人输入团队码即锁定属地；只能**改名**，改区域/改码需删除重建；删除后已绑定发音人属地保留。省管理员仅能管理本省团队码（越省 403）。
- 小程序端：登录完成步骤未绑定则**必填团队码**（`POST /api/mp/team/join`），绑定后**不可更换**（需后台纠错）；首页「未加入团队」提示条 + 绑定弹窗；任务/画像/我的页属地**只读**展示「省·市」。

**服务端强制隔离（客户端无法绕过）**：
| 接口 | 未绑定 | 跨区 |
|---|---|---|
| `GET /api/mp/tasks` | 空列表 `{total:0}` | 服务端按省+市+区县过滤，根本不返回 |
...
**发音人属地纠错**（「发音人管理」编辑弹窗）：管理员可改发音人省+市+区县（省管理员仅限本省、不能越省改；可给本省「未绑定」发音人补属地），**改属地自动清空 `team_code`**（原绑定作废，发音人可重新绑定/后台重定属地）；列表新增「属地」「团队码」列。
...
**任务分配关联团队码**（「任务分配」页，阶段八·C）：创建/编辑任务可**关联团队**——选团队后**投放区划由团队码自动带出**（省+市+区县随团队属地锁定），列表新增「关联团队」列。团队码不存则 422；省管理员只能关联本省团队（403）；地区与团队不一致 422。`task_batches` 新增 `team_code`（幂等迁移 `backend/scripts/migrate_task_team_code.py`）。**关联仅作归属追溯/筛选；可见性由任务自身的省/市/区县决定**（历史市级团队码区县为空 → 按全市投放）。
...
见 `docs/miniprogram-api.md`。已实现：微信登录 `POST /api/mp/login`、**协议三勾选 + 强制确认（阶段九）**、**团队绑定（属地=省+市+区县）`POST /api/mp/team/join`**、领任务 `GET /api/mp/tasks`、词条列表 `GET /api/mp/tasks/{id}/words`、录音上传 `POST /api/mp/recordings`、任务进度 `GET /api/mp/recordings/progress`、**总体进度 `GET /api/mp/progress`**、省/市列表 `GET /api/mp/regions?parent_code=`、发音人画像 `POST /api/mp/profile`、录音审核 `GET /api/review/recordings` + `POST /api/review/recordings/{id}/verdict`、数据集导出 `GET /api/review/export`、**我的录音时长统计与导出 `GET /api/mp/me/durations` + `GET /api/mp/me/export`**。**阶段八隔离**：任务/词条/上传按团队绑定属地（省+市+区县）强制过滤，未绑定=空任务、跨区=403。**阶段九协议守卫**：未同意最新版三份协议前功能接口统一 403。
