...
for s in test_api verify_agreements verify_cos_mode verify_dashboard verify_demo_task \
         ...
         verify_audit_log verify_quality_check verify_data_health verify_deadline_workload; do
...
> 推 master 后 **GitHub Actions 自动跑 CI**（`ci.yml`）：后端 19 脚本全量回归（一次性 postgres 库，不碰真实数据）+ 前端 `npm run build`。到仓库 **Actions** 页看是否全绿，红了先修再部署。
...
- [ ] GitHub Actions **CI** 全绿了吗（19 脚本回归 + 前端 build，红了先修再部署）
