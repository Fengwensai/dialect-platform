...
## 5. 小程序有改动 → 上传新版本 + 提审 + 发布

```bash
# 本地开发者工具
#  1. 详情 → 本地设置 → 勾选「不校验合法域名」
#  2. 真机预览/体验版自己先跑通（launch-check.md 清单）
#  3. 工具栏「上传」→ 版本号（如 1.0.1）+ 备注本次改动
# 浏览器打开 mp.weixin.qq.com
#  4. 版本管理 → 把刚传的开发版「提交审核」（人工 1~7 天）
#  5. 审核通过后 → 手动点「发布」才真正上线
```

**迭代期技巧**：
- 自己/内部测试用**开发版 / 体验版**（无需审核），只在要正式发布时才提审。
