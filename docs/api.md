...
> **`team_code` 关联语义（阶段八）**：传入团队码时，团队须存在（否则 422「团队码不存在」）；**投放区划由团队码自动带出**——`province_code`/`city_code` 必须与团队属地一致，否则 422「任务地区与团队码地区不一致，选择团队后地区由团队码自动带出」；`district_code` 一并由团队码带出（历史**市级团队**码区县为空 → 任务按全市投放）。省管理员只能关联本省团队码（403「只能关联本省的团队码」）。`team_code` 本身不参与可见性判定，可见性由任务的省/市/区县决定（见 `docs/miniprogram-api.md`）。
