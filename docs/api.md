...
**行为**：级联删除 `task_batch_items` 中的引用，避免孤儿数据。**已有录音的词条不允许删除**（`recordings` 无 FK 引用，硬删会留孤儿行）——后台完善 6 起拦截。

**错误**：`404 {"detail": "词条不存在"}`、`403 {"detail": "无权操作其他省份词条"}`、`400 {"detail": "该词条已有 N 条录音，不能删除"}`
...
### 4.6 批量启用/禁用词条（后台完善 6）

`POST /api/words/batch-status`

| 参数 | 说明 |
|---|---|
| `word_ids` | 要操作的词条 id 列表（去重保序；空 400；超 500 个 422） |
| `status` | `active` 启用 / `disabled` 禁用；非法值 422 |

**响应 200**：`{"processed": n, "skipped": m}`。`processed`=实际改状态的条数，`skipped`=已处于目标状态 / 不在本省范围的条数。

**行为**：省管理员自动跳过非本省词条（不报 403）；已是目标状态的跳过。**全跳过返回 400**（`"所选词条均无需操作（已处于目标状态或不在本省范围）"`）。单条审计日志（action=`批量启用词条`/`批量禁用词条`，detail 含 processed/skipped）。

**错误**：`400` 空列表 / 全跳过、`404` 词条不存在（`"词条不存在: [ids]"`）、`422` 非法 status / 超上限、`401` 未登录。
...
| 参数 | 说明 |
|---|---|
| `word_ids` | 要删除的词条 id 列表（去重保序；空 400；超 500 个 422） |

**响应 200**：`{"processed": n, "skipped": m}`。`processed`=实际删除的条数，`skipped`=有录音被跳过 / 不在本省范围的条数。

**行为**：**已有录音的词条跳过不删**（不连带删录音，避免无 FK 下留孤儿行），只删「干净的」；干净词条照单条删除清理 `task_claims` + `task_batch_items` 引用后删除。省管理员自动跳过非本省词条。**全跳过返回 400**（`"所选词条均无法删除（已有录音或不在本省范围）"`）。单条审计日志（action=`批量删除词条`）。

**错误**：`400` 空列表 / 全跳过、`404` 词条不存在（`"词条不存在: [ids]"`）、`422` 超上限、`401` 未登录。
...
> **`team_code` 关联语义（阶段八）**：传入团队码时，团队须存在（否则 422「团队码不存在」）；**投放区划由团队码自动带出**——`province_code`/`city_code` 必须与团队属地一致，否则 422「任务地区与团队码地区不一致，选择团队后地区由团队码自动带出」；`district_code` 被**强制清空**，即关联团队的任务一律按**全市**投放（不限定区县）。省管理员只能关联本省团队码（403「只能关联本省的团队码」）。`team_code` 本身不参与可见性判定，可见性由任务的省/市/区县决定（关联时区县为空 → 全市可见，见 `docs/miniprogram-api.md`）。
...
发音人即小程序端使用者。后台可**分页查看/筛选**发音人（含性别/年龄段画像与录音数），**编辑画像**（含**属地纠错**：省+市+区县），并**行内查看单个发音人的录音明细**（列表 + 审核状态分布 + 贡献统计）。

- **属地（省+市+区县）是阶段八隔离的核心**：由小程序端团队码绑定写入（见 `docs/miniprogram-api.md`），后台只读展示；发音人的任务可见性/录音权限严格按省+市+区县隔离。
...
- 昵称、device_id、openid 等不可改；前端弹窗可改性别/年龄段 + 属地（省+市+区县）。
- **属地纠错**：`province_code` 须为一级省级码，`city_code` 须为归属该省的二级市级码，`district_code` 须为归属该市的区县级码（任一级非法即 `422`）；省管理员不能把属地改到本省之外（`403`），但可给本省「未绑定」发音人补省+市+区县。
...
**一码一区县（省+市+区）**：每个团队码唯一绑定一个省+市+区县（`UNIQUE(province_code, city_code, district_code)`，**三级必选**）。发音人输入团队码即绑定该省市区县属地，随后只能看到/录制本区县（及本市未限定区县）的任务（阶段八隔离，见 `docs/miniprogram-api.md` §团队绑定）。历史团队码 `district_code` 为空，视为**市级团队**（全市可见）。
...
```json
{ "code": "HB-SJZ", "name": "石家庄团队", "province_code": "13", "city_code": "1301", "district_code": "130102" }
```

- `code` 必填，自动 `strip` + **转大写**存储。
- `province_code` / `city_code` / `district_code` **三级必选**，须为有效的一级省 / 二级市 / 三级区县码，且**逐级归属**（市须属该省、区县须属该市），否则 `422`。
- 省管理员只能创建本省团队码（越省 `403`）。

**错误**：
- `400 {"detail": "团队码已存在"}`（码重复）
- `400 {"detail": "该省市区县已有团队码（一码一区县），如需更换请删除后重建"}`
- `422 {"detail": "团队码不能为空"}` / `422 {"detail": "province_code 无效，须为有效省级代码"}` / `422 {"detail": "city_code 无效，须为归属该省的市级代码"}` / `422 {"detail": "district_code 无效，须为归属该市的区县级代码"}`
...
## 13. audit-logs — 操作审计日志（后台完善项 6）

> 仅 `super_admin` 可调用（省管理员 `403 {"detail": "需要超级管理员权限"}`），未登录 `401`。前端 `/audit-logs` 路由为 `superOnly`。
...
| `keyword` | 模糊匹配 `admin_name`（管理员名）或 `summary`（摘要） |
| `action` | 操作动词精确匹配（如 `审核驳回`、`合并发音人`） |
| `admin_id` | 按操作者过滤 |
| `start` / `end` | 时间区间（ISO 8601，如 `2026-08-01T00:00:00Z`；naive 按 UTC 处理） |

**响应 200**：按 `created_at` 倒序、`id` 倒序分页。
...
**错误**：`422 {"detail": "开始时间不能晚于结束时间"}`（`start` 晚于 `end`）。
...
2. POST /api/mp/team/join（小程序端）       → 发音人绑团队码，属地=省+市+区县
